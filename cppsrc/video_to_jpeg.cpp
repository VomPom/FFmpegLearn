...
int video_to_jpeg::run(string mp4_path, string output_dir) {
    int ret;
    ...
    in_filename = mp4_path.c_str();
    out_filename = output_dir.c_str(); //Change to your file path.
    ...
    while (true) {
        ...
        if (ret >= 0) { // Return the next frame of a stream.
...
int video_to_jpeg::saveJpg(AVFrame *pFrame, char *out_name) {
