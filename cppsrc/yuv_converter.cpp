...
int YuvConverter::yuv2mp4(string yuv_file_path, string des_file_path, int yuv_w, int yuv_h) {
    ...
    FILE *in_file = fopen(yuv_file_path.c_str(), "rb");
    if (!in_file) {
        LOGE("can not open file!");
        return -1;
    }
    int frame_num = 150;
    const char *out_file = des_file_path.c_str();    //Output file
    ...
    int codec_t = avcodec_open2(pCodecCtx, pCodec, nullptr);
    if (codec_t < 0) {
    ...
    for (int i = 0; i < frame_num; i++) {
        ...
        av_frame->pts = i;
        if (!avcodec_is_open(pCodecCtx) || !av_codec_is_encoder(pCodecCtx->codec)) {
            printf("test");
        } else {
            printf("ok");
        }
        int t = avcodec_is_open(pCodecCtx);
        // 编码一帧视频。即将AVFrame（存储YUV像素数据）
        // 编码为AVPacket（存储H.264等格式的码流数据）。
        int ret = avcodec_send_frame(pCodecCtx, av_frame);

        if (ret < 0) {
            printf(" av_err2str(ret):%s\n", av_err2str(ret));
            return -1;
        }
        ...
        if (got_picture == 0) {
            ...
            LOGE("encoder failed! code:%d %s", i, av_err2str(got_picture));
        }
    }
